...
const baseMsgRequestLoan: object = { creator: '', amount: '', fee: '', collateral: '', deadline: '' }
...
export const MsgLiquidateLoanResponse = {
  encode(_: MsgLiquidateLoanResponse, writer: Writer = Writer.create()): Writer {
    return writer
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgLiquidateLoanResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgLiquidateLoanResponse } as MsgLiquidateLoanResponse
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        default:
